...
<details>
	<summary> Логика обработки head, сгенерированного CGI </summary>

* Если имя запрашиваемого скрипта начинается с "nph-", то на создателе скрипта лежит ответственность за генерацию синтаксически правильного ответа. Другими словами, вывод из cgi не интерпретируется
* Если имя запрашиваемого скрипта НЕ начинается с "nph-", то сгенерированный хедер подлежит обязательной интерпретации со стороны сервера:
	* Если CGI скрипт сгенерировал первую строку ответа, то сервер её подменяет по своему усмотрению.
		- Скрипт вправе выставить собственный `статус-код`
		- версия `http` будет выставлена сервером принудительно.
		- `status text` будет заменен сервером соответственно `статус-коду`
	* CGI скрипт обязан сгенерировать поле `Content-type:`
	* CGI скрипт обязан сгенерировать поле `Location`, если возвращается не объект, а ссылка на него
	* Поле `Content-lenght` сервер выставляет самостоятельно
	* Все остальные поля, если они противоречат полям из сервера, будут заменены
